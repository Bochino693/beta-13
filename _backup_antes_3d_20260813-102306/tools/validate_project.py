...
def main() -> None:
    ...
    required_scripts = {
        "scripts/autoload/audio_synth.gd",
        "scripts/autoload/creature_db.gd",
        "scripts/autoload/game_state.gd",
        "scripts/autoload/move_db.gd",
        "scripts/components/battle_arena_3d.gd",
        "scripts/components/beast_combatant_3d.gd",
        "scripts/components/move_projectile_3d.gd",
        "scripts/scenes/battle.gd",
        "scripts/ui/ui_factory.gd",
    }
    for relative in sorted(required_scripts):
        if not (ROOT / relative).is_file():
            errors.append(f"Script obrigatorio ausente: {relative}.")

    minimum_scripts = 22
    if counts["scripts"] < minimum_scripts:
        errors.append(
            f"Contagem de scripts: {counts['scripts']}; minimo {minimum_scripts}."
        )
    elif counts["scripts"] > minimum_scripts:
        warnings.append(
            f"{counts['scripts'] - minimum_scripts} script(s) adicional(is) preservado(s)."
        )

...
- viewport vertical `720×1280`, estádio e combatentes volumétricos em `World3D`, câmera em perspectiva, luzes, público e sombras;
- 30 retratos mestres HD e 30 spritesheets 4×4, sem fallback de baixa definição;
- contrato para modelos GLB com animações de idle, ataque, dano, esquiva, celebração e derrota, mais proxies 3D durante a modelagem;
...
- música, ambiência de estádio, esquiva e 30 rugidos individuais locais, sem downloads ou tráfego durante a partida.
...
|---|---|---|
| Mover/posição na arena | W A S D | Setas |
...
Na arena há exatamente três posições: esquerda, centro e direita. Durante o aviso de um ataque, o defensor pode mover apenas uma posição. Sair da faixa visada reduz o dano, mas não o anula: golpes rápidos, técnicos, de controle e pesados mantêm fatores de raspão próprios. Cada um dos 80 golpes lança seu spritesheet exclusivo como energia dentro do espaço 3D; o corpo da Beast nunca é um retrato frontal ou billboard.

Modelos finais entram em `assets/models/beasts/<id>/<id>.glb`. Enquanto uma Beast ainda não possui GLB, o jogo usa um corpo volumétrico procedural da família anatômica para que estádio, câmera, costas do jogador, deslocamento e projéteis possam ser testados sem fingir que uma imagem plana é 3D. Veja `docs/PIPELINE_3D_BEASTS.md`.

...
func _build_screen() -> void:
	...
	type_badge.position = Vector2(97, 68)
	type_badge.size = Vector2(210, 36)
	info_panel.add_child(type_badge)
	var rarity := CreatureDB.rarity_of(creature)
	var rarity_badge := UIFactory.badge(CreatureDB.rarity_label(rarity), CreatureDB.rarity_color(rarity))
	rarity_badge.position = Vector2(323, 68)
	rarity_badge.size = Vector2(210, 36)
	info_panel.add_child(rarity_badge)
	...
	var stats := UIFactory.label("PONTUAÇÃO J1  %05d     PONTUAÇÃO J2  %05d\nTURNOS  %02d     BEASTS RESTANTES  %d\nCARTA  %s\nARENA  %d VITÓRIAS EM %d BATALHAS" % [GameState.scores[0], GameState.scores[1], GameState.battle_summary.get("rounds", 0), GameState.battle_summary.get("remaining", 0), creature["card_code"], BeastRecords.wins(creature_id), BeastRecords.battles(creature_id)], 17, Color("eaf2ff"), HORIZONTAL_ALIGNMENT_CENTER)
